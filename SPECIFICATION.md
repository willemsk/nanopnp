...
NOTE (IF-05, QR-09): the CCP4 *write* side is conditional on the interpreter. CCP4 maps are
written through GridDataFormats' `MRC` exporter, which arrived in GridDataFormats 1.2 — a release
requiring Python ≥ 3.11, while QR-09 keeps this project's floor at 3.10. On 3.10 the resolver takes
GridDataFormats 1.0.2, whose exporter registry is `DX, PKL, PICKLE, PYTHON`: it reads OpenDX, MRC
and CCP4 and writes OpenDX, but cannot write CCP4. Ingestion — the direction every stage of this
pipeline actually needs — is therefore unconditional, and only the CCP4 write side narrows. The
implementation reports the gap rather than discovering it from a third-party traceback: the writable
formats are queryable, and a CCP4 write where the writer is absent aborts naming the format, the
installed version and the release that gained the writer (QR-12). Pinning GridDataFormats ≥ 1.2
...
| **VER-44** | Live convergence monitoring and field visualisation | The rung and the Newton step reach the shell through a structural hook carrying the residual and the undamped relative update as numbers, never recovered from a progress caption; the hook is not an input, asserted by running the same case watched and unwatched and requiring one artefact hash and one store entry; a rung whose model takes no Newton callback yields a rung record and no steps, and the plot shows it as such rather than interpolating a line across it; a solve served from the store is named as such rather than drawn as an empty plot; the plot draws no convergence threshold, the criterion of NUM-16 being per rung and disjunctive, and reports per rung which of the two tests closed it and whether any step was forced; the field viewer renders a solution restored through the stage-10 gate rather than an export, its field names and units come from the IF-07 attribute vocabulary rather than from `gui/`, the scene reaches the view as a file rather than a data URL, and a document that loaded without its renderer is reported as a diagnostic naming the renderer source rather than shown as a blank panel (IF-09, QR-11, FR-27, NUM-16, NUM-18, QR-12) |
